...
Stages 1 through 3 are complete. The approved v0.2 policy is frozen locally; all 30 partly AI-assisted human labels remain prior-version and stale. The revised five-case Groq smoke test completed with valid schemas and no API errors, but persistent policy inconsistencies block bulk generation. Three new records and the original problematic Groq run are excluded without rewriting history. The combined training manifest selects 11 Gemini + 2 Groq labels, with 254 unattempted examples and 3 held for review; development remains 0/80. These are machine labels, not independent human accuracy. Golden remains untouched; no classifier training or evaluation has run. See [the smoke follow-up](docs/machine_annotation.md#revised-bounded-smoke-test). Source-derived text stays ignored.
